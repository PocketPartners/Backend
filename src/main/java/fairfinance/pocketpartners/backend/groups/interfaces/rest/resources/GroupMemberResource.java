...
public record GroupMemberResource (Long groupId, Long userId, String name, String groupPhoto, String fullName, String email, String phoneNumber, java.util.Date joinedAt){
    public static Object fromCommandToResource(GroupMember groupMember) {
        return new GroupMemberResource(groupMember.getGroup().getId(), groupMember.getUser().getId(), groupMember.getGroup().getName(), groupMember.getGroup().getGroupPhoto(), groupMember.getUser().getFullName(), groupMember.getUser().getEmailAddress(), groupMember.getUser().getPhoneNumber(), groupMember.getJoinedAt());
    }
}
