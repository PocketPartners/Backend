...
public class CreateGroupMemberCommandFromResourceAssembler {
    public static GroupMemberResource fromCommandToResource(GroupMember resource) {
        return new GroupMemberResource(resource.getGroup().getId(), resource.getUserInformation().getId(), resource.getGroup().getName(), resource.getGroup().getGroupPhoto(), resource.getUserInformation().getFullName(), resource.getUserInformation().getEmailAddress(), resource.getUserInformation().getPhoneNumber(), resource.getJoinedAt());
    }
}
